...
- 상용/표준 비교분석(가독성 재구성판, 원본은 SSOT_Explorer_상용비교분석.md,
  v3 갱신 2026-08-17 — D-056, CI/스키마 격차해소+MCP서버 신규경쟁축+
  라우터5단계 반영):
...
[D-049] MCP 서버를 Claude Code에 실제 등록 — 프로젝트 스코프 `.mcp.json`
결정: O-011("MCP 서버를 실제 IDE에 등록")의 재논의 조건("붙여서 써보고
      싶어질 때")이 바로 다음 라운드에 충족돼 착수. 저장소 루트에
      `.mcp.json` 신설:
      ```json
      {"mcpServers": {"ssot-explorer": {"command": "python",
        "args": ["${CLAUDE_PROJECT_DIR:-.}/ssot_mcp_server.py"]}}}
      ```
      **절대경로 안 씀** — 이 저장소는 D-039/D-040에서 이미 공개 전환하며
      개인정보/절대경로를 지운 공개 저장소라, 여기에 `C:\Users\yhs01\...`
      같은 개인 절대경로를 다시 박으면 그 작업을 무의미하게 만듦(같은
      맥락의 선례: 커밋 `9e781e7`가 개인 launcher 절대경로를 gitignore로
      뺀 것과 정확히 같은 우려). Claude Code 공식 문서(code.claude.com/
      docs/en/mcp)를 실제로 확인한 결과, 프로젝트 스코프 `.mcp.json`에서는
      `${CLAUDE_PROJECT_DIR}`를 맨몸으로 못 쓰고(그건 플러그인 제공 설정
      전용) `${CLAUDE_PROJECT_DIR:-.}`처럼 기본값을 같이 줘야 한다는 걸
      확인 후 그대로 반영 — 추측으로 문법을 짜지 않음.
      `command`도 절대경로(`C:\...\python.exe`) 대신 `"python"`만 씀 —
      README가 이미 "PATH에 python 있다"를 전제로 안내하고 있어(`pip
      install -r requirements.txt` 후 `python main.py`) 기존 가정과 통일.
이유: 등록 자체는 파일 하나 추가로 끝나는 저비용 작업이라 미룰 이유가
      없었고, 마침 실측(docs 확인)까지 필요해서 "짐작으로 문법 짜서
      나중에 깨짐" 리스크를 이번에 없앴다.
검증: `.mcp.json`이 유효한 JSON인지 파싱 확인. 이 저장소 안에 개인
      절대경로가 새로 추가되지 않았는지 grep으로 재확인. **아직 안 한 것**:
      실제로 이 파일을 Claude Code가 읽어 도구 승인 프롬프트를 띄우고
      `/mcp`에서 연결 상태로 보이는지는 세션 재시작이 필요해 이번 응답
      안에서는 직접 확인 못 함 — 다음 세션 시작 시 확인 필요(O-011 갱신,
      아래).

[D-050] "맥락형 인덱싱"을 MCP로 노출 — classify_content tool 추가
결정: 사용자가 "맥락형 인덱싱 부분은 어떻게 설계잡으면 될까?" 질문 →
      현황(키워드 레지스트리/임베딩 스켈레톤/세션 로그, D-044/D-045)이
      전부 GUI(SaveDocumentDialog)/CLI 안에서만 닫혀 있어 D-048/D-049의
      MCP 방향과 안 물려 있음을 짚고 설계 제안 → AskUserQuestion으로 범위
      확정: "classify_content만(신호만, 로깅 없음)" — 신뢰폐루프 피드백
      기록용 tool(record_classification_feedback)은 이번엔 안 만듦(MCP엔
      "승인 버튼" 이벤트 자체가 없어 새 설계가 필요한 지점이라 판단, 스켈레톤
      우선 원칙 유지).
      구현: ssot_mcp_server.py에 classify_content(text) -> dict 추가 —
      기존 router_orchestrator.orchestrate()(5단계 파이프라인, D-032~
      D-044)를 그대로 감싸기만 함, 새 분류 로직 0줄. router_orchestrator는
      main.py와 달리 Qt 미의존 순수 모듈이라 top-level import 가능(지연
      import 불필요, main.load_roots만 지연 import).
      **"파일 조작 없음" 범위를 문서화**: 프로젝트 파일(README/CLAUDE.md)은
      안 건드리지만, orchestrate()가 내부적으로 오케스트레이션 로그
      (ssot_orchestrator_log.json)와 키워드 레지스트리(ssot_keyword_
      registry.json)엔 계속 기록을 쌓는다 — 이건 D-044부터 GUI/CLI가
      이미 하던 동작이라 MCP 경유라고 새로 생긴 부작용이 아님을 tool
      docstring에 명시해 오해 방지.
      P-01 조건부 예외(SaveDocumentDialog의 "승인 버튼")가 이 경로에는
      아예 필요 없어짐 — 이 서버가 애초에 프로젝트 파일을 저장 안 하므로
      "승인 있으면 쓰기 가능" 예외 자체가 적용될 일이 없음.
이유: 이미 검증된 오케스트레이터를 그대로 재사용해 저비용으로 "맥락형
      인덱싱"과 "범용 IDE 플러그인" 두 축을 연결. 피드백 로깅까지
      가져가지 않은 건, MCP에서 "승인/거부"를 어떻게 포착할지가 이번
      라운드로 가볍게 결정하기엔 너무 큰 설계 질문이라 판단(O-012로 분리
      기록).
검증: 신규 테스트 4개(매칭 루트 1순위, 빈 레지스트리 시 빈 후보, 반환값
      JSON 직렬화 가능, 내부 로그가 실제로 쌓이는지) — test_ssot_mcp_
      server.py 전체 12→16개, 전체 pytest 181→185개 통과. 실제 프로세스
      스모크(echo "" | python ssot_mcp_server.py)도 재확인.

[O-012] MCP 경유 classify_content 호출의 신뢰 폐루프 데이터 수집 방법.
임시결정: 이번 라운드는 신호만 반환, 별도 피드백 기록 tool 없음(D-050).
MCP엔 GUI의 "승인 버튼" 같은 명시적 이벤트가 없어서, O-006/007/008이
기다리는 acceptance_rate() 데이터를 여기서도 모으려면 새 메커니즘이 필요 —
후보 두 갈래만 기록해둠(택 1 확정 안 됨):
(a) record_classification_feedback(text, chosen_label) tool을 별도로
  만들어 에이전트가 자발적으로 호출하게 유도(강제 안 됨, 응답률 미지수)
(b) 아예 안 모으고 GUI/CLI 경로의 기존 승인 데이터만 계속 씀(MCP는 순수
  조회 전용으로 남김)
재논의 조건: MCP 경유 호출량이 실제로 늘어서 "이 채널의 신호도 갖고
싶다"는 필요가 확인될 때.
...
[D-052] 등록 루트 경로 삭제/이동 감지 — 드리프트 로그 + 개발자 탭 + MCP
3곳에 같은 신호 노출
결정: 사용자가 폴더별 CLAUDE.md/README 구조를 MCP로 재점검하다가 "레지스
      트리에 등록됐는데 실제 폴더가 없어지면 어떻게 되나" 질문 → 기존
      드리프트 스크립트가 이미 `root_path.exists()`를 체크는 하면서도
      콘솔 출력(`(경로 없음, 건너뜀)`)에만 그치고 로그/알림엔 전혀 안
      남기고 있었음을 실측 확인. **자동 등록해제는 안 함** — 이 프로젝트가
      드리프트 감지(D-019)/InboxWatcher(D-042)/MCP(D-048) 전부 일관되게
      지켜온 "감지→알림만, 실제 변경은 사람 승인" 원칙을 그대로 따름.
      구현 3곳(같은 신호, 채널만 다름):
      - `ssot_index_drift_check.py`(SSOT_Explorer 레포 밖,
        `~/.claude/scripts/`) — `missing_root_lines()` 신설, `review_
        staleness_lines()`와 같은 자리에서 같은 패턴으로 diff_lines에
        합류. 매일 09:00 스케줄 실행 + "지금 드리프트 체크 실행" 버튼
        둘 다에서 자동으로 걸림.
      - `main.py`의 `format_registry_text()` — 루트 블록 헤더에 `⚠️경로
        없음` 태그(🌐웹정본 태그와 같은 자리) — 개발자 탭 레지스트리 뷰에서
        바로 보임.
      - `ssot_mcp_server.py`의 `list_ssot_roots()` — 각 항목에
        `pathExists: bool` 필드 추가 — MCP로 조회하는 에이전트도 같은
        신호를 받음(D-050 "맥락형 인덱싱을 MCP로" 연장선).
이유: 세 채널(스크립트 로그/GUI/MCP)이 전부 같은 레지스트리를 보고 있는데
      이 신호만 스크립트 콘솔에 갇혀 있었던 게 일관성이 없었음 — 이미
      있던 `exists()` 체크 하나를 세 군데 얇게 노출하는 저비용 작업이라
      한 라운드에 같이 처리.
검증: 드리프트 스크립트는 별도 pytest 스위트가 없어(레포 밖 파일) 스크래치
      레지스트리(존재/없음 루트 각 1개)로 2회 실행 — 최초실행(베이스라인)
      이후 재실행에서 `missing_root_lines()` 로그 줄 실제 생성 확인.
...
[D-056] 상용비교분석 v3 재조사·아티팩트 갱신 (D-037~D-055 반영)
결정: `SSOT_Explorer_상용비교분석.md`(v2, 2026-08-14, D-001~D-036 대상)가
      3일치 19개 결정(D-037~D-055)만큼 드리프트됐음을 먼저 분석해서
      사용자에게 보고(CI/스키마 서술이 사실과 다름, pytest 94→192 숫자
      노후, MCP 서버라는 완전히 새로운 경쟁축 누락) → 사용자가 전체
      재조사(v3)를 지시, 갱신은 md(설계도, 정본)에만 반영하고 웹
      아티팩트는 기존 하드코딩 URL(115070d7-...)에 그대로 갱신 발행
      하라고 명시.
      WebSearch로 4개 항목 재검증: (1) Ruler(intellectronica) — v0.3.44
      계열에서 "에이전트별 MCP 서버 설정" 등으로 MCP 설정배포 기능이
      더 심화됨(방향은 여전히 "배포"라 SSOT_Explorer의 신규 MCP 서버와
      정반대, 직접 경쟁 아님) (2) opcode/Claudia — GitHub 릴리스 페이지
      직접 재확인, v0.2.0(2025-08-31) 이후 신규 릴리스 없음 재확인(정체
      "7개월"→"1년 가까이"로 연장) (3) AGENTS.md — 60,000개+/20개+ 툴
      숫자 재확인, 드리프트 없음 (4) "repo-registry를 MCP로 노출"하는
      패턴은 가이드 문서 수준에서만 개념적으로 언급, 구체적 경쟁 제품은
      이번에도 특정 못 함(라우터 섹션과 같은 정직성 톤 유지).
      md 갱신 내용: 축A(CI/스키마 — v2가 "격차 큼"으로 지적한 두 항목이
      D-038에서 실제로 생김, verdict를 "격차 좁혀짐"으로 조정), 축B
      (라우터 3→5단계 확장 D-044 반영, MCP 서버로 자기 데이터 노출이라는
      신규 섹션 추가, Inbox감시/개발자콘솔 내부UX 신규 섹션 추가), 종합/
      나아갈방향 섹션 전면 재작성, 출처 3개 추가([20][21][22]). 내부
      리팩터(D-053~055, H-009/010/011)는 경쟁 포지셔닝에 영향 없다고
      판단해 본문에서 제외(동시성/로깅 섹션에 한 줄 참고만).
      웹 아티팩트: 기존 발행본을 WebFetch로 먼저 읽어 CSS/디자인 시스템
      (톤, verdict 배지 4종, item/fresh 카드 패턴)을 그대로 파악한 뒤
      같은 토큰·클래스로 컨텐츠만 갱신 — artifact-design 스킬 원칙("이미
      있는 디자인 시스템을 먼저 찾아서 따른다")대로 새 디자인을 만들지
      않고 기존 것을 재사용. 같은 URL로 republish해 링크 유지.
이유: 사용자가 "분석 먼저"를 명시적으로 요청 — 드리프트 여부/범위를
      먼저 확인하지 않고 바로 갱신에 들어가면 어느 정도 낡았는지도
      모른 채 작업 범위를 임의로 정하게 됨. 분석 결과를 보고한 뒤
      AskUserQuestion으로 갱신 범위(가벼운 정정/전체재조사/보류)를
      사용자가 직접 고르게 한 것도 "정직성 조건"(추측 대신 확인) 원칙의
      연장 — 범위 자체도 추측하지 않음.
검증: md 파일 전체 재작성 후 목차/버전배지/기준일/변경이력 4곳 모두
      v3/2026-08-17/D-055로 일관되게 갱신됐는지 재확인. 웹 아티팩트는
      publish 성공 응답으로 같은 URL 유지 확인(새 아티팩트 생성 아님).
      Ruler/opcode 릴리스 페이지는 WebFetch로 직접 재확인(추측 아닌
      실측 — 특히 opcode "7개월"→"1년" 표현은 실제 릴리스 날짜
      2025-08-31과 오늘 날짜 2026-08-17을 직접 계산해서 나온 수치).

[D-057] 개발자 모드 토글 신설 — O-010(개발자 콘솔 웹서빙 2단계) 대체
결정: 사용자가 O-010의 2단계(웹서빙 전환) 자체를 재논의 — "MCP가 이미
      IDE 어디서든 조회를 해결하는데 웹서버가 왜 필요하냐"는 지적을
      받아들여 웹서빙 계획을 폐기(O-010 항목에 기록). 대신 레지스트리
      최상위 `developerMode`(bool, 기본 True — "이 앱을 쓰면 이미
      개발자"가 기본 전제지만 항상 맞진 않을 수 있어 사용자가 직접 끌 수
      있게) 플래그 하나를 신설해 두 가지를 동시에 게이팅:
      (1) main.py — 툴바에 체크 가능한 "개발자 모드" 액션 신설(항상
      보임, 꺼도 되돌릴 경로가 항상 있음). 꺼지면 "개발자" 탭을
      QTabWidget에서 떼어내고(ManagementPanel 인스턴스는 유지 — 다시
      켜면 상태 안 잃고 재부착), "개발자 탭으로" 툴바 버튼도 비활성화.
      (2) ssot_mcp_server.py — `list_ssot_roots`/`check_readme_freshness`/
      `classify_content` 3개 tool 전부 호출 시작 시 같은 플래그를 확인,
      꺼져 있으면 실제 로직을 안 돌리고 `{"error": "developer_mode_
      disabled", "message": "..."}` 하나만 반환(파일 조작 없음, 로그도
      안 쌓임).
      함수는 `router_proposals.is_developer_mode()`/`set_developer_mode()`
      로 신설 — 두 프로세스(GUI/MCP 서버)가 같은 레지스트리 파일에서
      같은 플래그를 읽어야 해서 Qt 미의존 모듈에 둠(router_orchestrator.
      orchestrate()의 log_path 패턴처럼 registry_path를 명시 인자로 받아
      테스트 격리 + main.py의 REGISTRY_PATH 재사용 둘 다 지원).
      REGISTRY_SCHEMA(D-038)에도 `developerMode: boolean` 필드 추가.
이유: "개발자 콘솔이 어디 있든 Claude Code가 MCP로 확인만 하면 된다"는
      사용자 통찰이 O-010의 전제(웹서빙 없인 GUI 밖에서 개발자 데이터를
      못 본다) 자체를 무효화 — MCP 서버(D-048~050)가 이미 그 역할을
      하고 있어 포트/인증/exe패키징 같은 O-010의 미결 4개를 붙잡고 있을
      이유가 없어짐. 대신 "누구나 이 앱을 쓴다고 개발자는 아니다"라는
      더 실제적인 문제(사용자 설정 vs 개발자 설정 분리)가 남아있었고,
      이건 웹서빙보다 훨씬 작은 토글 하나로 풀렸다.
검증: pytest 신규 13개(router_proposals 6개 — 기본값True/필드없음/명시
      False/set round-trip/다른 키 보존/tmp미잔존, ssot_mcp_server 4개 —
      3개 tool 게이팅 + 명시적True 정상동작, main.py 3개 — 시작부터
      꺼진 레지스트리로 열면 탭 없음+액션 미체크+버튼 비활성, 런타임
      토글로 탭 탈부착+레지스트리 반영, 다시 켜면 같은 패널 인스턴스로
...
[D-063] 컨트롤플레인을 flutter_App\.claude\에서 Plug_In_Global\.claude\로
이전(소급 기록)
결정: 레지스트리(ssot-roots.json)+상시로드 작업규칙+설계방법론 문서+
      New_App_Env_Kit 전체를 `SSOT_Coding_File\Plug_In_Global\.claude\`로
      물리 이동(사용자가 직접 폴더 복사 후 구 폴더 삭제). 전역 훅 2개
      (`~/.claude/hooks/ssot_session_context.py`, `ssot_index_reminder.py`)
      의 REGISTRY_PATH를 새 경로로 갱신 + `SSOT_REGISTRY_PATH` 환경변수
      우선순위 패턴(D-039에서 앱만 적용돼 있던 것)을 두 훅에도 동일하게
      적용해서 드리프트 제거. 사용자 전역 CLAUDE.md의 `@import` 경로도
      갱신(겸사겸사, 원래 `.claude\.claude\` 이중폴더를 가리키던 깨진
      경로였던 것도 같이 바로잡음). Windows User 환경변수
      `SSOT_REGISTRY_PATH` 자체도 새 경로로 갱신(구 값이 이미 삭제된
      flutter_App 경로를 가리키고 있어서 실제로 stale 상태였음, 실측
      확인). `referenceCondition_작성_템플릿.md`(신규 — 골격+README 체인
      규칙 문서화) 작성, `ssot-roots.json` 자체 편집도 PostToolUse
      리마인더가 감지하도록 확장.
이유: 컨트롤플레인이 여러 루트가 공유하는 것인데 자식 프로젝트(flutter_App)
      하나 안에 물리적으로 얹혀 있는 게 구조적으로 어색했음 — 부모
      워크스페이스(SSOT_Coding_File) 밑에 전용 허브 폴더를 두는 쪽이
      맞다고 판단(사용자 제안).
검증: 전역 훅 2개를 REGISTRY_PATH 갱신 후 실제 실행해서 flutter_App 매치
      정상 확인. Windows 환경변수 갱신 후 `[System.Environment]::
      GetEnvironmentVariable`로 재확인. **이 세션 자체는 갱신 전
      프로세스라 옛 환경변수 값을 계속 참조** — 다음 세션부터 실제 반영.
비고: 이 항목은 실제 작업 완료 후 소급 기록됐다(이 프로젝트 자신의
      "구조 변경은 D-번호로 기록한다" 원칙을 그 순간엔 안 지켰다는 뜻 —
      D-063 자체가 그 지적을 받아 정리하며 기록됨).

[D-064] 단어 역할 태깅(target/mention) — "언급 vs 소유" 혼동(O-007/008)
1차 완화 + AI 판단 스켈레톤 신설
결정: `router_classifier.tag_word_roles()` 신설 — kiwipiepy 품사+위치
      정보로 매치된 단어마다 target/mention/neutral을 매겨 `classify_
      content()` 점수에 반영(TARGET_ROLE_BONUS=0.15 가산,
      MENTION_ROLE_PENALTY=0.15 감산, 둘 다 additive — D-033 원칙 유지).
      패턴 2개만 실측 확인 후 채택: ①매치어 바로 뒤(또는 의존명사 하나
      건너)에 목적지 조사(에/로/으로/에다)가 오면 target ②대조 마커
      (말고/대신/아니라) 앞의 최근 매치어=mention, 뒤=target. 반환에
      `matchedKeywordRoles` 필드 추가(기존 필드는 안 바뀜, 추가만).
      별도로 `router_ai_judgment.py` 신설(router_embeddings.py와 완전히
      같은 스켈레톤 패턴) — 실제 LLM 판단으로 후보를 재순위하는 경로는
      틀만 만들고 `AIJudgeProviderNotConfigured`만 던짐. orchestrator에
      4.5단계로 연결(semantic 단계와 동일하게 항상 skipped 기록), 파이프
      라인 5단계→6단계.
이유: 순수 키워드 겹침은 "flutter_App 말고 Coding_Nomal에 넣어"류 문장에서
      배경 언급과 실제 요청 대상을 구분 못 함(O-007/008). 사용자가
      AskUserQuestion으로 "오프라인 유지+신호 보강"과 "오프라인 원칙 깨고
      실제 AI 연결" 중 전자를 지금 채택하되 후자는 O-009와 같은 방식으로
      스켈레톤만 미리 만들어두기로 명시적으로 갈랐음(2026-08-19).
검증: kiwipiepy를 직접 실행해서 "말고"가 VV"말"+EC"고", "아니라"가
      VCN"아니"+EC"라"로 쪼개진다는 걸 먼저 확인(리터럴 문자열 매치로
      짰으면 조용히 무효과였을 케이스) — 이 실측 결과를 회귀 테스트로
      고정. 실제 등록 레지스트리로 라이브 쿼리 실행해서 "coding_nomal이
      flutter_App보다 위로" 원하는 방향 확인. pytest 246개(기존 237 +
      신규 9) 전부 통과.
[한계 발견, 정직하게 기록] 라이브 검증 중 실제 버그 발견 — role은 단어
      문자열 하나당 하나만 기억해서, "flutter_App"과 "Local_APP"처럼
      서로 다른 라벨이 "app" 같은 하위 토큰을 공유하면 위치와 무관하게
      같은 role이 두 라벨에 다 적용됨("...coding_nomal 말고 local_app에
      넣어야지"에서 진짜 target인 뒤쪽 "app"(local_app 소속)의 target
      판정이 배경 언급이던 앞쪽 "app"(flutter_app 소속)에도 잘못 묻어남).
      **당초 D-033과 같은 이유로 O-015로 미루려 했으나, 사용자가 "지금
      고칠까?"라고 재확인해서 같은 날 바로 착수 — D-065 참고, O-015는
      그 결정으로 확정됨.**
관련 D-번호: D-029(union구조/CLI), D-033(IDF+additive), D-034(kiwipiepy),
      D-044(오케스트레이터 5단계), O-007/O-008/O-009, D-065.

[D-065] O-015 수정 — role을 단어 문자열이 아니라 루트별 위치로 판정
(D-064 발견 직후, 같은 날 즉시 수정)
결정: `tag_word_roles(text, words) -> dict[word, role]`(전역, 단어
      문자열 단위)를 폐기하고 `label_role(tokens, root) -> role`(루트별,
      위치 단위)로 교체. 각 루트가 자기 label이 원문 어디서 매치됐는지
      먼저 위치를 찾고(`_find_label_span`), 그 위치 기준으로만 목적지
      조사/대조마커를 본다 — 같은 "app" 조각을 공유하는 두 라벨이 있어도
      서로 다른 위치의 매치는 서로 다른 role을 받는다.
이유: D-064 라이브 검증 중 "flutter_App"/"Local_APP"이 "app"을 공유해서
      role이 뒤섞이는 걸 발견 — 처음엔 O-015로 미루려 했으나, 실제로
      다시 보니 수정 범위가 예상(_keyword_signal 전체 재작성)보다 작았음
      (`classify_content`의 채택 로직 `_keyword_signal`은 그대로 두고,
      role 계산 함수만 위치 인식형으로 바꾸면 충분했음) — 사용자가 "지금
      고칠까?"라고 재확인해서 그 자리에서 바로 착수.
검증: TDD로 진행 — 회귀 테스트(`test_label_role_shared_subtoken_does_not_
      bleed_across_labels`)를 먼저 작성해서 레드 확인, 구현 후에도 처음엔
      실패(2차 버그: `_find_label_span`이 label_words[0]을 원문 맨 앞
      3토큰 안에서만 찾아서, "말고" 뒤에 나오는 라벨을 전부 놓침 —
      "flutter_app 말고 coding_nomal에" 같은 흔한 문장이 전부 실패했음).
      원인 파악 후 label_words[0] 탐색을 원문 전체로 넓히는 걸로 수정,
      재실행해서 그린 확인. pytest 248개(D-064 246 + 신규 2) 전부 통과.
      실제 등록 레지스트리로 다시 라이브 쿼리 실행 — Local_APP 0.804(role
      전부 target) > flutter_App 0.654(role 전부 neutral)로 원하는 순위
      확인.
관련 D-번호: D-064, O-015(이 결정으로 확정됨 — 아래 O-015 항목 참고).

[D-066] on_selection_changed(뷰어 렌더링) 테스트 신설 — Qt 시각 회귀 갭
1차 대응
결정: `on_selection_changed()`(D-001부터 이 앱 존재 이유였던 "트리 선택→
      CLAUDE.md/README.md 뷰어 표시" 로직)가 테스트 0개였던 걸 발견하고
      7개 신설: 등록루트인데 인덱스파일 없음/미등록 하위폴더/파일1개만/
      파일2개(구분선)/파일 선택 시 힌트, 5가지 분기 커버. 픽셀 스크린샷
      diff는 폰트 렌더링差로 환경마다 깨지기 쉬워(false positive) 이
      프로젝트의 결정적 테스트 원칙(D-024)과 안 맞아서 채택 안 함 — 대신
      `viewer.toPlainText()`/`toMarkdown()`으로 실제 렌더 결과 텍스트를
      확인하는 속성 기반 테스트로.
이유: Part1 진단 아티팩트(2026-08-19)가 "Qt 렌더링 자체를 겨냥한 테스트가
      구조적으로 없다"고 지적한 것 중 가장 핵심적인 사례 — 사용자가 그
      갭부터 먼저 메우기로 확정.
검증: 작성 중 실제 버그성 이슈 2건 발견(둘 다 테스트 자체의 잘못된 가정,
      프로덕션 버그 아님) — ① `toPlainText()`로 마크다운 구분선("---")을
      찾으려 했는데, 렌더러가 이미 실제 수평선 요소로 바꿔서 평문엔 안
      남음 → `toMarkdown()`(원본 마크다운 소스로 왕복)으로 교체, Qt가
      "---"를 "- - -"로 정규화한다는 것도 실측으로 확인. ②
      _ensure_all_roots_initialized()(D-031, RootInitWorker)가 앱 시작 시
      등록된 빈 루트에 자동으로 init CLAUDE.md를 만든다는 걸 놓쳐서, "아예
      비어있음" 케이스가 타이밍에 따라 있다/없다 갈리는 비결정적 테스트가
      됐었음 — `root_init_worker.wait()`로 명시적으로 기다린 뒤 일부러
      지워서 재현하는 걸로 수정. pytest 253개(D-065 248 + 신규 5) 전부
      통과, 연속 5회 재실행으로 QThread 타이밍 비결정성 없음 확인.
[비고] 남은 Qt 커버리지 갭: SearchDialog 자체(검색 UI 상호작용), 컨텍스트
      메뉴 액션 각각의 실제 동작(존재 여부만 테스트됨), 크로스플랫폼
      (Mac/Linux) 실기 미검증 — 전부 O-번호로 남기지 않고 다음에 순서
      정할 때 다시 논의(범위가 애매해서 지금 O-번호로 확정 안 함).
관련 D-번호: D-001(뷰어가 이 앱의 핵심), D-024(결정적 테스트 원칙),
      D-031/D-053(RootInitWorker).

[D-067] O-009 확정 — 임베딩 프로바이더를 로컬 모델(fastembed)로 실제 연결
결정: 사용자에게 AskUserQuestion으로 "로컬 모델(추천)/Gemini API/OpenAI
      API/지금은 보류" 4개 중 선택을 요청 — 로컬 모델 선택. `fastembed`
      (Qdrant, ONNX 런타임 — torch 의존성 없음)로 로컬 실행, requirements.
      txt에 `fastembed==0.8.0` 고정 추가. 애초 계획한
      `intfloat/multilingual-e5-small`(query/passage 프리픽스로 embed_
      text/embed_query_text 분리와 정확히 들어맞을 것으로 기대)은
      `TextEmbedding.list_supported_models()`로 fastembed 자체 레지스트리를
      실측하니 large(2.24GB)뿐이라 배포 크기상 부적합 — `sentence-
      transformers/paraphrase-multilingual-MiniLM-L12-v2`(0.22GB, 한국어
      포함 50+ 언어)로 교체. 이 모델은 프리픽스 관례가 없어 embed_text/
      embed_query_text가 현재는 동일 동작(인터페이스는 향후 프리픽스형
      모델 교체 가능성 위해 분리 유지). 모델 로딩은 지연 초기화+락으로
      스레드 세이프 캐싱. fastembed 미설치 시(선택적 의존성 취급)
      EmbeddingProviderNotConfigured로 안내 — orchestrator의 기존 except
      절이 하위호환으로 계속 동작.
이유: 이 프로젝트는 D-034부터 완전 오프라인 원칙을 지켜왔다 — Gemini/
      OpenAI API는 그 원칙을 처음 깨는 결정이라 O-009가 명시한 재논의
      조건대로 사용자 동의가 먼저 필요했다. 로컬 모델은 API 키/네트워크
      호출/비용/외부 텍스트 전송이 전부 없어 원칙을 안 깬다.
검증: 실제 임베딩 계산으로 스팟체크(모델 로드+최초 추론 226.85초 — HuggingFace
      Hub에서 가중치 최초 다운로드 포함, 이후 캐시돼 0.2초/건) —
      "flutter_App은 Flutter 앱들을 모아둔 프로젝트다"(passage) 대비
      "플러터 앱 관련 작업"(관련 query) 코사인 유사도 0.6782, "오늘 점심
      메뉴 추천"(무관 query) 0.1468. 옛 DEFAULT_MIN_SIMILARITY=0.7(Lazzy/
      Gemini 실측 이식값)로는 **관련 쌍마저 걸러졌을 것** — 0.5로 낮춤(O-016,
      표본 2쌍뿐인 잠정값).
      fastembed 0.8.0이 "이 모델은 이제 CLS 대신 mean pooling을 쓴다"는
      런타임 경고를 냄(라이브러리 쪽 동작 변경, 우리 코드 아님) — 향후
      fastembed가 다시 바뀌면 벡터 분포가 조용히 달라질 수 있어 버전을
      0.8.0으로 고정.
      orchestrator/mcp_server/main 세 테스트 파일이 전부 router_orchestrator.
      orchestrate()를 거치므로, 실제 모델을 매번 띄우지 않도록 각 파일의
      기존 autouse 격리 픽스처에 embed_query_text 스텁(기본 "미연결" 경로
      유지)을 추가 — router_orchestrator.py의 semantic 단계가 결과 랭킹에
      아직 관여하지 않는다는(D-044 설계 그대로) 사실도 재확인, 실제 연결
      시 skipped=False로 바뀌는 것만 새 테스트로 별도 검증. pytest 257개
      전부 통과(D-066 253 + 신규 4).
[비고] semantic 단계는 이제 embed_query_text를 실제로 호출해 skipped=False
      까지는 기록하지만, 그 결과(임베딩 벡터)를 후보 재순위에 실제로 쓰는
      로직은 아직 없다(router_orchestrator.py 195번째 줄 주석 참고) —
      "프로바이더 연결"과 "그 결과를 랭킹 신호로 편입"은 별개 작업으로
      의도적으로 분리해뒀다(O-016 다음 라운드 후보).
...
[D-076] 개발자 탭에 분류 파이프라인 벤치마크 신설 — "샌드박스 후 실제
코드" 질문에 "이 기능은 애초에 안전해서 구분 자체가 불필요" 결론
결정: `router_orchestrator.orchestrate()`에 스테이지별 소요시간(`elapsedMs`,
      `time.perf_counter()` 기준)+전체 합산(`totalElapsedMs`) 추가(6개
      스테이지 전부, `_log_run`이 이미 쌓던 `ssot_orchestrator_log.json`에도
      같이 저장됨 — 별도 로그 스키마 신설 없음). `ManagementPanel`(개발자
      탭)에 "분류 파이프라인 벤치마크" 절 신설 — 텍스트 입력+"지금 실행"
      버튼을 누르면 기존 `ClassificationWorker`(D-051/H-008, SaveDocumentDialog
      가 쓰던 QThread 래퍼)를 그대로 재사용해 실제 등록 레지스트리로
      `orchestrate()`를 배경 스레드에서 실행하고, 결과(스테이지별 ms+상위
      후보 3개)를 보여준다. 같은 절 위에 "오케스트레이션 로그(최근 20건)"
      뷰도 신설 — GUI든 CLI든 MCP든 어느 경로로 classify가 불렸든 이미
      쌓이던 로그를 처음으로 눈에 보이게 함.
      **"샌드박스처럼 실제 코드도 붙이면 좋겠다"는 요청에 대한 판단**:
      별도 샌드박스 계층을 안 만들었다 — `classify_content`/`orchestrate()`
      는 애초에 P-01(읽기전용, 프로젝트 파일 절대 안 건드림) 설계라 실제
      등록 레지스트리로 바로 실행해도 안전하고, 자기 자신의 내부 로그에만
      기록을 남긴다(D-044부터 이미 그래왔음, MCP로 옮겨서 새로 생긴 동작도
      아님). "샌드박스 먼저, 검증되면 실제 코드"라는 절차가 필요한 경우는
      신뢰 못 하는 임의 스크립트를 실행할 때(액션 레지스트리 policy=auto,
      D-058/O-013)뿐이고, 그건 청사진 Phase 6(액션 샌드박스화)의 범위이지
      이 기능과는 다른 문제.
이유: 사용자가 지난 라운드에서 제안한 "코드/인프라 쪽 포트폴리오 개선"
      6개 중 확정한 것들(#1 CI 게이트는 D-075) 다음으로 "#6 벤치마크를
      개발자 탭에서" + "실제 코드 실행"을 요청. 드리프트체크가 이미
      QProcess로 "버튼 눌러서 실제 스크립트 실행"을 하고 있었지만, 그건
      무거운 서브프로세스 패턴이라 순수 함수 호출인 classify엔 안 맞아서
      더 가벼운 기존 QThread 워커를 재사용.
검증: pytest 전체 333개 통과(신규 6개 — elapsedMs/totalElapsedMs 존재확인,
      format_orchestration_log_text 최신순 표시+후보없음 케이스,
      ManagementPanel 빈 로그뷰, 빈 입력시 워커 안 뜸, 실제 파이프라인
      실행 후 결과뷰+로그뷰 둘 다 갱신되는지). 이 과정에서 router_
      orchestrator.py의 `steps = []`(타입힌트 없음)가 mypy를
      `sum(s.get("elapsedMs", 0) for s in steps)` 줄에서 헷갈리게 만드는
      걸 실제로 잡아냄(`list[dict]`로 명시해서 해결) — D-075에서 만든
      게이트가 신규 코드에서 바로 값을 낸 첫 사례.
관련 D-번호: D-075(이번에 켠 mypy 게이트가 실제로 잡은 문제), D-051/H-008
      (ClassificationWorker 원조), D-013(SearchWorker — QThread 배경작업
      패턴 원조), D-074(개발자 탭 확장 선례 — labeledFolders GUI 노출).

[D-077] `docs/index.html` — 읽기전용 정적 웹 데모 신설(GitHub Pages 대상)
결정: 클론/설치 없이 브라우저에서 3초 만에 뭔지 이해하게 하는 정적
      페이지. 기존 `dev_console_static/dev_console.html`(D-046, 로컬
      FastAPI 서버가 실데이터를 서빙하는 라이브 콘솔)의 시각 언어(테일
      그린 accent, light/dark `prefers-color-scheme` 대응, `el()` DOM
      헬퍼)를 그대로 재사용 — 새 디자인 시스템을 또 만들지 않고 "이 앱의
      실제 브랜드"를 이어감. 다른 점: 백엔드 fetch(`/api/*`) 대신 예시
      데이터(가상의 Acme_Mobile_App 등, 전부 fictional)를 파일 안에 직접
      내장 — GitHub Pages는 정적 호스팅이라 서버가 없음. 상단
      탐색기/개발자 2탭 구조로 실제 GUI(main.py QTabWidget)를 그대로
      미러링: 탐색기는 트리+README 뷰어, 개발자는 레지스트리 요약/스키마
      검증/라벨폴더 감사상태(D-073)/분류 파이프라인 벤치마크 예시
      결과(D-076)까지 실제 앱의 4개 신규 기능을 전부 한 페이지에 압축.
      **라벨 폴더 감사 카운트다운은 하드코딩 안 하고 실시간 계산** —
      `lastAudited`는 고정 달력 날짜로 박아두고, `daysRemaining`은 방문자의
      실제 브라우저 `Date()` 기준으로 `router_registry.labeled_folder_
      audit_status()`와 동일한 로직을 JS로 재구현해서 계산 — D-073의
      "세션마다 재계산" 메커니즘이 데모에서도 진짜로 재현됨(단, 데이터를
      안 갱신하면 몇 달 뒤엔 전부 "due"로 보이는 게 의도된 자연스러운
      결과 — 페이지 안 주석에도 명시).
      **"읽기 전용"을 글자 그대로 지킴**: 벤치마크 절에 "지금 실행" 같은
      가짜 버튼을 안 넣었다 — 눌러도 아무것도 안 하는 버튼은 사용자를
      속이는 UX라, 과거 예시 실행 결과를 고정 표시만 하고 "실제 앱에선
      여기서 진짜 실행합니다"라고 명시.
      GitHub Pages 활성화(Settings → Pages → Source: Deploy from branch →
      master → /docs)는 저장소 설정 화면 조작이라 이 세션이 못 함 — 사람이
      한 번 눌러야 함(gh CLI 미설치 확인됨).
이유: 지난 라운드에서 정리한 코드/인프라 포트폴리오 개선 후보(#1 CI는
      D-075, #6 벤치마크는 D-076) 중 #5(읽기전용 웹 데모)를 이어서 진행 —
      "클론 없이 이해 가능"이 원래 제안 취지.
검증: jsdom으로 실제 DOM 렌더링 검증(탭 전환, 트리 클릭 선택, 라벨폴더
      경고배지 렌더링까지 시뮬레이션) — `Archived_Client_Notes`
      (lastAudited 2026-07-01, 오늘 기준 53일 경과)가 실제로 "⚠️ 30일 감사
      주기 도달" 배지로 렌더링되는지 확인. `node --check`로 인라인 JS
      구문 검증. 임시로 설치한 jsdom/npm 산출물(node_modules 등)은 검증
      후 정리, 레포에 안 남음.
관련 D-번호: D-046(dev_console_static 시각 언어 원조), D-073(라벨폴더 감사
      로직 재구현 대상), D-076(벤치마크 예시 데이터 출처), D-075/D-076
      (직전 두 라운드의 "코드/인프라 포트폴리오" 후보 목록 원조).

[D-078] 포지셔닝 결정(소급 기록) — "상용화 아키텍처 전환" 대신 "GitHub
공개 포트폴리오"로 확정, D-075~077은 이 결정의 실행분
결정: SSOT_Explorer를 실제 상용 제품으로 키우는 방향은 보류하고, 지금
      코드베이스를 그대로 GitHub에 공개해 포트폴리오로 쓰는 방향으로
      확정. 이 라운드에서 만든 산출물 2개(claude.ai 아티팩트, 비공개 —
      README에 링크만, 실제 공유는 사용자가 필요할 때 켬):
      - **SSOT_Explorer 해부도**(전체 기능 흐름 다이어그램 + Backstage/
        Cortex·OpsLevel/Cursor·Copilot·Windsurf 3갈래와 10축 비교분석,
        솔직한 강점/약점 포함)
      - **상용화 청사진**(그래도 상용화한다면 거칠 7단계 아키텍처
        로드맵 + Phase 2 인증/RBAC 상세설계 — 실행 계획이 아니라 "이
        방향을 안 가는 이유"를 뒷받침하는 분석 문서로 기능)
      이후 실제로 착수한 건 상용화 트랙이 아니라 "코드/인프라 포트폴리오
      개선" 6개 후보 목록이었고, 그중 확정된 것들이 D-075(CI 게이트)/
      D-076(개발자 탭 벤치마크)/D-077(읽기전용 웹 데모)로 실행됨.
이유: 비교분석에서 확인된 세 가지가 상용화 보류의 근거: (1) 진짜 차별점
      (4포맷 규칙 동기화 + 세션 주입 + 승인폐루프 조합)이 Claude Code/
      MCP라는 남의 플랫폼 기능 표면 위에 얹혀 있어, 플랫폼이 네이티브로
      흡수하면(가능성 낮지 않음) 하루아침에 무의미해질 수 있는 구조적
      플랫폼 리스크, (2) 메커니즘 자체(레지스트리→규칙파일 생성+세션주입)
      가 기술적으로 어렵지 않아 자금 있는 경쟁자가 몇 주 안에 재현
      가능 — 지금 모트는 "먼저 만들었다"뿐, (3) 분류 파이프라인 6단계 중
      2단계(시맨틱/AI판단)가 여전히 스켈레톤이라 "똑똑한 라우팅"이라는
      세일즈 포인트를 지금 팔면 오버클레임. 반대로 포트폴리오 방향은
      리스크가 거의 없음 — 실제 코드 스캔(ruff/mypy/bandit)해보니 개인
      경로/이메일 하드코딩이 전혀 없어(전부 `Path.home()`+환경변수 기반)
      스크럽할 것도 없었고, 오히려 결정이력 74개+테스트 300개대+MCP
      프로토콜 실전 통합이 대부분의 포트폴리오엔 없는 "판단 과정이
      보이는" 신호로 작동.
검증: 별도 코드 실행 없음(포지셔닝 판단 자체는 대화 안에서 완결, 실제
      검증은 그 결과로 나온 D-075~077 각각의 검증란 참고). 이 항목은
      D-075 작업 착수 전에 대화로 이미 확정됐던 걸 이번에 소급 기록—
      D-001(범용규칙 결정이력)의 소급 기록 관례와 동일.
관련 D-번호: D-075, D-076, D-077(전부 이 결정의 실행분), D-072(포지셔닝
      논의에서 참고한 "Backstage catalog-info.yaml 패턴 의도적 차용"
      배경 — owner/scope/lastReviewed 스키마 원조).
...
**D-067로 확정됨(2026-08-21)** — 재논의 조건 (2)가 충족됨: 사용자가
AskUserQuestion으로 로컬 모델을 명시적으로 선택해 오프라인 원칙을 안
깨는 형태로 연결. 상세는 D-067, MIN_SIMILARITY 재보정은 O-016 참고.
관련 D-번호: D-034(오프라인 원칙 확립), D-044, D-067.

[O-010] 개발자 콘솔(dev_console_server.py, D-046)을 실제로 main.py에 통합.
**2026-08-17(D-057) 재논의 완료 — 2단계(웹서빙) 폐기, 대체안으로 대체됨**:
사용자가 "MCP가 이미 IDE가 어디서든 조회하는 문제를 풀었는데 별도
웹서버가 왜 필요하냐"고 지적 — D-048~050의 MCP 서버가 정확히 O-010이
2단계로 미루고 있던 "이 앱을 안 열어도 개발자 콘솔 데이터를 볼 수
있어야 한다"는 목표를 이미 달성했음을 재확인. 웹서빙(포트/인증/exe
패키징 미결 항목 4개)은 전부 폐기 — 대신 D-057(개발자 모드 토글)로
같은 문제를 더 간단하게 풀었다: 개발자 탭은 계속 GUI 안에 두되(1단계
그대로), 레지스트리 최상위 `developerMode` 플래그로 탭 표시/MCP 서버
tool 3개를 껐다 켰다 할 수 있게 함 — "이 앱을 쓰면 이미 개발자"라는
전제가 항상 맞진 않을 수 있다는 사용자 지적 반영(기본값 True는 유지).
상세는 D-057 본문 참고.
~~임시결정(폐기): 사용자가 명시한 2단계 로드맵 — 1단계(D-047, 완료): 앱 안
상시 "개발자" 탭으로 먼저 보여주기. 2단계(폐기): "나머지 환경이 세팅되면"
웹서빙으로 전환.~~
관련 D-번호: D-042(InboxWatcherThread 패턴), D-043(Qt 미의존 리팩터 선례),
D-046, D-048~050(MCP 서버가 2단계 목표를 대체 달성), D-057(대체 구현).
...
[O-014] router_ai_judgment.py의 실제 AI 판단 프로바이더 연결(Gemini/Claude
API 등) — O-009와 같은 이유로 오프라인 원칙과 정면 충돌.
임시결정: 틀만 구축(D-064) — judge_candidates()는 항상
`AIJudgeProviderNotConfigured`.
재논의 조건: O-009와 동일(사용자가 프로바이더를 명시적으로 정하고 API
키/네트워크 호출+텍스트 외부 전송에 동의할 때) — 또는 tag_word_roles
(D-064)만으로 O-007/008이 데이터로 안 풀린다는 게 확인될 때.
관련 D-번호: D-034(오프라인 원칙), D-044/O-009(임베딩 스켈레톤과 동일
패턴), D-064.

[O-015] tag_word_roles(D-064)가 단어 문자열 단위로만 role을 기억해서,
서로 다른 루트 라벨이 같은 하위 토큰을 공유하면(예: "_App"/"_APP" 접미사
공유) role이 위치와 무관하게 뒤섞이는 한계.
**D-065으로 확정됨(2026-08-19, 같은 날)** — 당초 "이번 라운드 범위 밖"으로
적었으나, 사용자가 그 자리에서 "지금 고칠까?"라고 재확인했고 실제
수정범위가 예상보다 작아서(role 계산 함수만 교체, `_keyword_signal`
재작성 불필요) 즉시 착수·완료. 상세는 D-065 참고.
관련 D-번호: D-064, D-065.

[O-016] router_embeddings.DEFAULT_MIN_SIMILARITY=0.5(D-067 재보정)가 표본
2쌍(관련 1 + 무관 1)짜리 잠정값 — 실사용 쿼리로 재보정 필요. 또한
semantic 단계(4단계)가 embed_query_text를 실제로 호출은 하지만(skipped
기록용) 그 결과를 후보 재순위에 반영하는 로직은 아직 없음(router_
orchestrator.py 195번째 줄) — "프로바이더 연결"과 "랭킹 신호로 편입"을
D-067에서 의도적으로 분리해둔 나머지 절반.
임시결정: 두 가지 다 지금은 미착수. MIN_SIMILARITY는 0.5로만 낮춰두고
실사용 로그가 쌓이길 기다림(이 프로젝트의 LOW_SAMPLE_THRESHOLD 관례와
같은 정신).
재논의 조건: (1) 실제 classify_content 호출이 어느 정도 쌓여 0.5가
과다/과소 매칭인지 판단할 표본이 생길 때 (2) 임베딩을 실제 랭킹 신호로
편입하기로 하면 — 기존 키워드/IDF 점수와 어떤 가중치로 합칠지(additive?
...
**D-079로 (a)안도 구현 완료(2026-08-23)** — (b)가 먼저 채택됐지만 (a)가
풀려던 문제(프로즈 매칭에만 맡기면 README를 안 열어보고 지나치는 사례)는
(b)의 라벨-폴더 감사와는 별개 트랙이라 그대로 남아있었음 —
`ssot_session_context.py`에 `_find_readme()`(router_orchestrator.
_find_readme와 동일하게 플랫 루트/`.claude` 하위 두 위치만, 대소문자
무시) 추가, `build_context()`가 매치된 루트에 README.md가 있으면 프로즈
매칭 여부와 무관하게 "📄 이 루트의 README.md: {경로} — 먼저 열어볼 것"
줄을 항상 포함하도록 수정. 매치된 루트 1개만 검사(전체 트리 스캔 없음,
비용 거의 0 유지). 실측 검증: flutter_App(README 없는 루트, D-011에서
확인됨)은 줄 자체가 안 뜸, Local_APP(README 있음)은 정상 노출 — 두 경우
다 기대대로 동작 확인. (a)(b) 둘 다 완료로 O-018 종결 — 남은 건 D-073이
범위 밖으로 남긴 "실제 이동추적 재탐색 알고리즘"뿐(필요해지면 별도 항목).
...
**추가 확인(2026-08-24, 같은 날 — MCP 디렉토리 제출 시도 중 발견)**: 공식
MCP Registry(modelcontextprotocol.io/registry)는 PyPI(또는 npm/Docker 등)
**배포가 필수 전제조건**임을 확인 — `server.json`이 실제 PyPI 패키지를
가리켜야 하고, 그 패키지 README에 `<!-- mcp-name: io.github.<user>/
<server> -->` 소유권 마커까지 있어야 검증 통과. 즉 이 O-019가 끝나야만
MCP 레지스트리 제출(별도 항목화 안 함, 이 O-019의 후속 작업으로만 추적)이
가능한 순서 의존 관계 — "필요성 낮음"이 아니라 "블로킹당함"으로 재분류.
사용자가 PyPI 배포 보류 결정(2026-08-24) → MCP 레지스트리 제출도 같이
보류.
관련 D-번호: 없음(신규 항목, 아직 D로 승격 안 됨 — 실행 전까지 O-번호로만
추적).

[D-080] pytest-cov 커버리지 게이트 + README 배지 도입 (2026-08-24)
결정: `[tool.coverage.run]`(source=전체, shibokensupport 등 PySide6
런타임 부트스트랩 파일 omit)/`[tool.coverage.report]`를 pyproject.toml에
추가. CI `pytest` 잡을 `--cov --cov-report=term-missing
--cov-fail-under=75`로 변경(실측 80%, 333 tests에서 5%p 버퍼 —
MIN_SIMILARITY 재보정과 동일한 "안전선 먼저, 나중에 재조정" 원칙).
README에 정적 배지(80%, shields.io) 추가.
이유: 포트폴리오 공개 준비 라운드(D-077/D-078) 다음 단계로 사용자가
직접 요청 — "테스트 295개 있다"보다 "커버리지 몇 %다"가 더 정량적인
품질 신호. main.py(GUI)도 측정 대상에 포함(ruff/mypy가 스텁 오탐으로
제외한 것과 달리, test_main.py가 QT offscreen으로 실제로 그 경로를
실행하므로 커버리지는 뺄 이유 없음).
알려진 한계: README 배지는 정적 텍스트라 실제 커버리지가 바뀌어도
자동 갱신 안 됨(CI 게이트 자체는 자동 — 75% 밑으로 떨어지면 빌드
실패로 여전히 잡힘). 커밋마다 자동 갱신되는 동적 배지(예: 커버리지
SVG를 CI가 커밋)는 write-back 권한 등 인프라가 더 필요해 이번 라운드
범위 밖 — 배지 수치가 실측과 눈에 띄게 벌어지면 그때 재검토.
관련 D-번호: D-077/D-078(포트폴리오 공개 라운드).

**후속(같은 날, 2026-08-24) — router_sync.py 갭 메우기**: D-080 도입
직후 실측 79.94%(router_sync.py 87%)로 확인됐던 걸, `resolve_claude_md_
target` 플랫경로 분기/`generate_full_export_pointer`의 web-참고전용
분기/`generate_init_readme_md`·`generate_full_export_readme_md`(전체
미검증 상태였음)/wrapper 함수 2개/`write_one_format`의 OSError 경로
(target 자리에 디렉토리를 만들어 실제 조건으로 재현, mock 없음) — 9개
테스트 추가로 router_sync.py 100% 달성. 전체 80.39%(342 tests)로 재확인.

[D-082] README를 MCP-first로 재구성 (2026-08-24)
결정: 첫 문단과 최상단을 "GUI 앱"이 아니라 "MCP 서버(범용 IDE 플러그인)"
로 리드하도록 재작성. 신규 "IDE에 연결하기" 섹션 — Claude Code/Cursor/
Windsurf 3개 설정 스니펫(D-081 실측 검증 근거 포함, PyPI 미배포 상태를
반영해 설치 명령은 로컬 클론 `pip install ".[mcp]"` 기준으로 유지, O-019
참고). GUI 관련 섹션은 "선택 사항인 로컬 뷰어"로 재프레이밍(`## 기능`
헤더 밑에 명시 문구 추가).
발견해서 같이 고친 낡은 정보 3건(모두 실측 재확인 후 정정): (1) MCP
tool 개수 "6개"→**7개**(`check_labeled_folders_audit` 서술에서 누락돼
있었음, `@server.tool()` grep으로 실제 개수 재확인) (2)
`classify_content`/CLI 진입점 설명의 파이프라인 단계수 "5단계"/"3단계"
→**6단계**(D-063 이후 실제 스테이지 6개 — `router_orchestrator.py`의
`"stage":` 키 6종 grep으로 재확인, README가 그 갱신을 안 따라갔던 것)
(3) O-011 상태 "Cursor/Windsurf 등엔 아직 안 붙임(필요성 미확인)"→
**D-081로 3개 IDE 전부 검증 완료**.
이유: "범용 IDE 플러그인" 포지셔닝(D-048)이 실제로는 README 첫인상에서
GUI 데스크톱 앱으로 읽혔음(사용자 지적) — MCP가 진짜 핵심인데 부산물
(GUI, 2026-08-12 최초 MVP)이 앞서 있던 순서를 바로잡음.
검증: 세션 종료 전 이번에 추가한 결정이력 항목(D-079~D-082, O-018/O-019
갱신분) 전체를 실제 코드/파일/PyPI 상태와 하나하나 대조 — 훅 파일의
`_find_readme` 존재, 커버리지 실측 재실행(80.39%/342 tests 일치),
Cursor/Windsurf mcp.json 파일 내용 실측 확인, tool 7개 grep 재확인,
PyPI 404 재확인, 파이프라인 6스테이지 재확인 — 전부 일치, 환각 없음
확인.
관련 D-번호: D-048("범용 IDE 플러그인" 원 결정), D-081(연결 검증 근거),
O-019(설치 명령이 클론 기준인 이유).
